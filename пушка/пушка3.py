...
class Gun:
    ...
    def fire2_end(self, event):
        ...
        global balls, bullet
        bullet += 1
        new_ball = Ball(self.screen, self.x, self.y) # передача координаты пушки
        new_ball.r += 5
...
target1 = Target()
target2 = Target()
# Позиционируем вторую мишень в другом месте
target2.x = rnd(400, 600)
target2.y = rnd(200, 400)
target2.r = rnd(15, 40)
finished = False
...
while not finished:
    ...
    gun.draw()
    target1.draw()
    target2.draw() # Рисуем вторую мишень
    for b in balls:
        b.draw()
    ...
    for b in balls:
        b.move()
        # Проверяем столкновение с первой мишенью
        if b.hittest(target1) and target1.live:
            target1.live = 0
            target1.hit()
            target1.new_target()
        # Проверяем столкновение со второй мишенью
